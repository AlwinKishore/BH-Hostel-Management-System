...
@section('header', 'Edit Furniture Item')
...
<div class="max-w-9xl mx-auto">
    ...
    <div class="glass-card p-10 border-none shadow-2xl shadow-slate-200/60 transition-transform duration-500 hover:scale-[1.01]">
        <div class="mb-10">
            <h3 class="text-2xl font-black text-slate-800 tracking-tight">Modify Furniture Profile</h3>
            <p class="text-sm text-slate-600 font-medium">Update asset details, relocation status, or condition reports</p>
        </div>

        <form action="{{ route('furniture.update', $furniture) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    ...
                    <input type="text" name="name" id="name" class="form-input-premium" placeholder="e.g. Wooden Bed Frame" value="{{ old('name', $furniture->name) }}" required>
                ...
                <div>
                    ...
                    <input type="text" name="code" id="code" class="form-input-premium" placeholder="e.g. FUR-001" value="{{ old('code', $furniture->code) }}" required>
                        ...
                        <div>
                            ...
                            <div class="relative group">
                                <select name="type" id="type" class="form-input-premium appearance-none" required>
                                    <option value="Bed" {{ old('type', $furniture->type) == 'Bed' ? 'selected' : '' }}>Bed</option>
                                    <option value="Table" {{ old('type', $furniture->type) == 'Table' ? 'selected' : '' }}>Table</option>
                                    <option value="Chair" {{ old('type', $furniture->type) == 'Chair' ? 'selected' : '' }}>Chair</option>
                                    <option value="Cupboard" {{ old('type', $furniture->type) == 'Cupboard' ? 'selected' : '' }}>Cupboard</option>
                                    <option value="Fan" {{ old('type', $furniture->type) == 'Fan' ? 'selected' : '' }}>Fan</option>
                                    <option value="Air Conditioner" {{ old('type', $furniture->type) == 'Air Conditioner' ? 'selected' : '' }}>Air Conditioner</option>
                                </select>
                            ...
                            <div class="relative group">
                                <select name="room_id" id="room_id" class="form-input-premium appearance-none">
                                    ...
                                    @foreach($rooms as $room)
                                        <option value="{{ $room->id }}" {{ old('room_id', $furniture->room_id) == $room->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="condition" id="condition" class="form-input-premium appearance-none">
                        <option value="new" {{ old('condition', $furniture->condition) == 'new' ? 'selected' : '' }}>Brand New</option>
                        <option value="good" {{ old('condition', $furniture->condition) == 'good' ? 'selected' : '' }}>Good / Serviceable</option>
                        <option value="damaged" {{ old('condition', $furniture->condition) == 'damaged' ? 'selected' : '' }}>Damaged</option>
                        <option value="repairable" {{ old('condition', $furniture->condition) == 'repairable' ? 'selected' : '' }}>Under Repair</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status" class="form-input-premium appearance-none">
                        <option value="available" {{ old('status', $furniture->status) == 'available' ? 'selected' : '' }}>Available for Assignment</option>
                        <option value="assigned" {{ old('status', $furniture->status) == 'assigned' ? 'selected' : '' }}>Already Assigned</option>
                        <option value="broken" {{ old('status', $furniture->status) == 'broken' ? 'selected' : '' }}>Broken / Out of Service</option>
                        <option value="maintenance" {{ old('status', $furniture->status) == 'maintenance' ? 'selected' : '' }}>Scheduled Maintenance</option>
                    </select>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end space-x-6 pt-10 border-t border-slate-100">
                ...
                <button type="submit" class="btn-premium px-10">
                    <i class="fas fa-check-double mr-2 opacity-70"></i> Update Furniture
                </button>
            </div>
        </form>
    </div>
