...
<div class="max-w-9xl mx-auto">
    <div class="flex items-center space-x-3 text-slate-500 mb-10 font-bold text-[10px] uppercase tracking-[0.2em]">
        ...
        <span>/</span>
        <span class="text-slate-800">New Receipt</span>
    </div>
    ...
    <div class="glass-card p-10 border-none shadow-2xl shadow-slate-200/60 transition-transform duration-500 hover:scale-[1.01]">
        <div class="mb-10 text-center md:text-left">
            <h3 class="text-2xl font-black text-slate-800 tracking-tight">Record Financial Transaction</h3>
            <p class="text-sm text-slate-600 font-medium">Capture student fee payments and generate automated reference tracking</p>
        </div>

        <form action="{{ route('payments.store') }}" method="POST" class="space-y-12">
            @csrf
                    ...
                    <div class="space-y-6">
                                ...
                                <select name="student_id" id="student_id" class="form-input-premium appearance-none" required>
                                    ...
                                    @foreach($students as $student)
                                        <option value="{{ $student->id }}" {{ old('student_id') == $student->id ? 'selected' : '' }}>
                                            {{ $student->name }} — ID: {{ $student->id_proof_number ?? 'N/A' }} (Loc: {{ $student->room?->room_number ?? 'In Inventory' }})
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="date" name="payment_date" id="payment_date" class="form-input-premium" value="{{ old('payment_date', date('Y-m-d')) }}" required>
                                @error('payment_date') <p class="mt-2 text-[10px] font-black uppercase text-rose-500 ml-4 tracking-wider">{{ $message }}</p> @enderror
                            </div>
                        </div>

                        <div>
                            <label for="transaction_id" class="form-label-premium">Bank Reference / TXN ID <span class="text-[9px] lowercase font-normal italic opacity-60 ml-1">(Optional)</span></label>
                            <input type="text" name="transaction_id" id="transaction_id" class="form-input-premium" placeholder="e.g. TXN99283100" value="{{ old('transaction_id') }}">
                ...
                <div class="space-y-8">
                    ...
                    <div class="space-y-6">
                            <div>
                                ...
                                <div class="relative group">
                                    <select name="payment_method" id="payment_method" class="form-input-premium appearance-none">
                                        <option value="Cash" {{ old('payment_method') == 'Cash' ? 'selected' : '' }}>Hard Cash</option>
                                        <option value="Bank Transfer" {{ old('payment_method') == 'Bank Transfer' ? 'selected' : '' }}>Direct Bank Transfer</option>
                                        <option value="Online (UPI/Card)" {{ old('payment_method') == 'Online (UPI/Card)' ? 'selected' : '' }}>UPI / Digital Card</option>
                        ...
                        <div>
                            ...
                            <textarea name="remarks" id="remarks" rows="4" class="form-input-premium resize-none" placeholder="Add specific notes about the transaction (e.g., partial refund, late fee included)">{{ old('remarks') }}</textarea>
            ...
            <div class="flex items-center justify-end space-x-6 pt-10 border-t border-slate-100">
                ...
                <button type="submit" class="btn-premium px-12 py-4 shadow-xl shadow-emerald-600/20">
                    <i class="fas fa-check-circle mr-2 opacity-70"></i> Post Transaction
                </button>
            </div>
        </form>
    </div>
