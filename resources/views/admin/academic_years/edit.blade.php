...
<div class="max-w-3xl mx-auto">
    <div class="flex items-center space-x-2 text-slate-500 mb-6 font-bold text-[10px] uppercase tracking-widest">
        ...
        <span>/</span>
        <span class="text-slate-800">Edit Academic Year</span>
    </div>

    <div class="glass-card p-10 border-none shadow-2xl shadow-slate-200/60">
        <div class="mb-10">
            <h3 class="text-2xl font-black text-slate-800 tracking-tight">Modify Academic Year Details</h3>
            <p class="text-sm text-slate-600 font-medium">Update the academic batch properties</p>
        </div>

        <form action="{{ route('academic-years.update', $academic_year) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="name" id="name" class="form-input-premium" placeholder="e.g. Academic Year 2026-2028" value="{{ old('name', $academic_year->name) }}" required>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        ...
                        <input type="date" name="start_date" id="start_date" class="form-input-premium" value="{{ old('start_date', $academic_year->start_date) }}" required>
                    ...
                    <div>
                        ...
                        <input type="date" name="end_date" id="end_date" class="form-input-premium" value="{{ old('end_date', $academic_year->end_date) }}" required>
                        @error('end_date') <p class="mt-2 text-[10px] font-black uppercase text-rose-500 ml-4 tracking-wider">{{ $message }}</p> @enderror
                    </div>
                </div>

                <div class="flex items-center mt-4">
                    <input type="checkbox" value="1" name="is_current" id="is_current" class="w-5 h-5 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500" {{ old('is_current', $academic_year->is_current) ? 'checked' : '' }}>
            ...
            <div class="flex items-center justify-end space-x-6 pt-10 border-t border-slate-100">
                ...
                <button type="submit" class="btn-premium px-10 py-4">
                    <i class="fas fa-save mr-2 opacity-70"></i> Update Academic Year
                </button>
            </div>
        </form>
    </div>
