        ...
        <form action="{{ route('years.update', $year) }}" method="POST" class="space-y-8">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <select name="batch_id" id="batch_id" class="form-input-premium">
                        ...
                        @foreach($batches as $batch)
                            <option value="{{ $batch->id }}" {{ old('batch_id', $year->batch_id) == $batch->id ? 'selected' : '' }}>
                                {{ $batch->batch_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex items-center mt-4">
                    <input type="checkbox" value="1" name="is_active" id="is_active" class="w-5 h-5 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500" {{ old('is_active', $year->is_active) ? 'checked' : '' }}>
