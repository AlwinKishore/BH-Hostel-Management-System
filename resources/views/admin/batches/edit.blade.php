...
<div class="max-w-3xl mx-auto">
    ...
    <div class="glass-card p-10 border-none shadow-2xl shadow-slate-200/60">
        <div class="mb-10">
            ...
            <p class="text-sm text-slate-600 font-medium">Update the batch properties</p>
        </div>
        ...
        <form action="{{ route('batches.update', $batch) }}" method="POST" class="space-y-8">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="batch_name" id="batch_name" class="form-input-premium" placeholder="e.g. 1st Batch" value="{{ old('batch_name', $batch->batch_name) }}" required>
                ...
                <div>
                    ...
                    <select name="academic_year_id" id="academic_year_id" class="form-input-premium" required>
                        ...
                        @foreach($academic_years as $academic_year)
                            <option value="{{ $academic_year->id }}" {{ old('academic_year_id', $batch->academic_year_id) == $academic_year->id ? 'selected' : '' }}>
                                {{ $academic_year->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex items-center mt-4">
                    <input type="checkbox" value="1" name="is_active" id="is_active" class="w-5 h-5 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500" {{ old('is_active', $batch->is_active) ? 'checked' : '' }}>
