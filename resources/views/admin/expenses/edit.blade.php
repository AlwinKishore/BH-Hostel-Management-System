...
@section('header', 'Update Expense Record')
...
<div class="max-w-9xl mx-auto">
    ...
    <div class="glass-card p-10 border-none shadow-2xl shadow-slate-200/60 transition-transform duration-500 hover:scale-[1.01]">
        <div class="mb-10">
            <h3 class="text-2xl font-black text-slate-800 tracking-tight">Modify Expense Entry</h3>
            <p class="text-sm text-slate-600 font-medium">Update transaction details, categories, or settlement status</p>
        </div>

        <form action="{{ route('expenses.update', $expense) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="title" id="title" class="form-input-premium" value="{{ old('title', $expense->title) }}" required>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
                        ...
                        <label class="cursor-pointer group">
                            <input type="radio" name="category" value="{{ $cat }}" class="hidden peer" {{ old('category', $expense->category) == $cat ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <div class="relative group">
                        ...
                        <input type="number" step="0.01" name="amount" id="amount" class="form-input-premium pl-12" value="{{ old('amount', $expense->amount) }}" required>
                    </div>
                </div>

                <div>
                    <label for="date" class="form-label-premium">Transaction Date</label>
                    <input type="date" name="date" id="date" class="form-input-premium" value="{{ old('date', $expense->date->format('Y-m-d')) }}" required>
                </div>

                <div>
                    <label for="paid_to" class="form-label-premium">Payee / Recipient Name</label>
                    <input type="text" name="paid_to" id="paid_to" class="form-input-premium" value="{{ old('paid_to', $expense->paid_to) }}">
                </div>

                <div>
                    <label for="payment_method" class="form-label-premium">Settlement Mode</label>
                    <input type="text" name="payment_method" id="payment_method" class="form-input-premium" value="{{ old('payment_method', $expense->payment_method) }}">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <label class="cursor-pointer group">
                            <input type="radio" name="status" value="paid" class="hidden peer" {{ old('status', $expense->status) == 'paid' ? 'checked' : '' }}>
                        ...
                        <label class="cursor-pointer group">
                            <input type="radio" name="status" value="pending" class="hidden peer" {{ old('status', $expense->status) == 'pending' ? 'checked' : '' }}>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3" class="form-input-premium py-4">{{ old('description', $expense->description) }}</textarea>
                </div>
            </div>
