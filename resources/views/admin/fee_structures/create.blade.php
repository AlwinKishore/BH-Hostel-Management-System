...
@section('header', 'Define Pricing Structure')
...
<div class="max-w-9xl mx-auto">
    <div class="flex items-center space-x-3 text-slate-500 mb-10 font-bold text-[10px] uppercase tracking-[0.2em]">
        ...
        <span>/</span>
        <span class="text-slate-800">New Structure</span>
    </div>

    <div class="glass-card p-10 border-none shadow-2xl shadow-slate-200/60">
        <div class="mb-10 text-center md:text-left">
            <h3 class="text-2xl font-black text-slate-800 tracking-tight">Configure Pricing Logic</h3>
            <p class="text-sm text-slate-600 font-medium">Standardize recurring charges for food, amenities, or specific room types</p>
        </div>

        <form action="{{ route('fee-structures.store') }}" method="POST" class="space-y-8">
            @csrf
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="md:col-span-2">
                    <label for="name" class="form-label-premium">Fee Nomenclature (Name)</label>
                    <input type="text" name="name" id="name" class="form-input-premium" placeholder="e.g. Standard Mess Fee, Premium WiFi, Security Deposit" value="{{ old('name') }}" required>
                    @error('name') <p class="mt-2 text-[10px] font-black uppercase text-rose-500 ml-4 tracking-wider">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="amount" class="form-label-premium">Monetary Value (Amount)</label>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <label class="cursor-pointer group">
                            <input type="radio" name="frequency" value="{{ $freq }}" class="hidden peer" {{ old('frequency') == $freq || ($loop->first && !old('frequency')) ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" name="room_type" id="room_type" class="form-input-premium" placeholder="e.g. All, Double, Luxury" value="{{ old('room_type') }}">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="flex flex-wrap gap-3">
                        <label class="cursor-pointer">
                            <input type="radio" name="building_id" value="" class="hidden peer" {{ !old('building_id') ? 'checked' : '' }}>
                        ...
                        <label class="cursor-pointer">
                            <input type="radio" name="building_id" value="{{ $building->id }}" class="hidden peer" {{ old('building_id') == $building->id ? 'checked' : '' }}>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="3" class="form-input-premium py-4" placeholder="Clarify what this fee covers...">{{ old('description') }}</textarea>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end space-x-6 pt-10 border-t border-slate-100">
                ...
                <button type="submit" class="btn-premium px-12 py-4">
                    <i class="fas fa-check-double mr-2 opacity-70"></i> Save Structure
                </button>
            </div>
        </form>
    </div>
